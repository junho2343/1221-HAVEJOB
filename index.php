...
<body>
	<div class="data_box first_box" ng-app="myApp" ng-controller="myjob">
		...
		<h1><a href="index.php"><img class="logo" src="./images/logo.png" alt="logo" width="300px"></a></h1>
			<form onsubmit="return false;" class="form">
<!--				<label for="job">채용검색</label>-->
				<input type="text" id="job" placeholder="검색내용을 넣어주세요(예: 웹/java)">
				<button type="submit" class="job_btn"></button>
				<input type="hidden" class="play_button" ng-click="page();">
			</form>
		...
		<div class="text_box dn">
			...
			<ul ng-repeat="job in jobs track by $index" ng-if="job.error != 0" >
				...
				<li><button class="location_button" data-id="{{ job.id }}" data-cnt="{{ job.cnt }}">자세히</button></li>
			</ul>
            <div class="page_box">
	...
	<script>
		...
		$(".job_btn").on("click",function(){
			...
			if(job_val == ''){
                ...
                da_b.children('div').not(".search_box").css({'padding-left':140});
                te_b.addClass('db');
                logo.css({
                    width:"150",
                    float:"left"
        ...
        $('#map_box button').on("click",function(){
            $('#map_box').addClass("dn");
        });
   $(".text_box").on("click",".location_button",function(){
      $("#map_box").removeClass('dn');
       ...
       console.log(data_id);
       $("#company_box").load('company.php?id='+company_id[data_cnt]);
        initialize(data_cnt);
   });
